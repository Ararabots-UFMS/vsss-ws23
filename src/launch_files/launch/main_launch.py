...
def evaluate_params(context, *args, **kwargs):
    ...
    message_server_arg = LaunchConfiguration('message_server').perform(context)
    message_server = message_server_dict.get(message_server_arg, "now_message_server")
    launch_nodes = [
        Node(
            package='vision',
    ...
    launch_nodes.append(
        RegisterEventHandler(
            OnProcessExit(
                ...
                on_exit=[
                    LogInfo(msg=(user_namespace, ' closed the main window')),
